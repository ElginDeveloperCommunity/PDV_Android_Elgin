...
final public class displayService {

    private final List<Produto> carrinho = new ArrayList<>();

    private final Activity mActivity;

    public displayService(Activity activityRef) {
        this(activityRef, null); // Chama o novo construtor com modelo nulo (opcional)
    }

    public displayService(Activity activityRef, String modelo) {
        mActivity = activityRef;

        if ("Tpro".equalsIgnoreCase(modelo)) {
            E1_Display.init(mActivity, E1_Display.DisplayDevices.TPRO);
        } else {
    ...
    public int ApresentaTextoColorido(String texto, int alinhamento, int tamanhoFonte, int posicaoVertical, int posicaoHorizontal, String cor) {
        return E1_Display.ApresentaTextoColorido(texto, alinhamento, tamanhoFonte, posicaoVertical, posicaoHorizontal, cor);
    }

    public int ApresentaImagemDisplay(Bitmap bitmap) {
        return E1_Display.ApresentaImagemDisplay(bitmap);
    }

    public int ApresentaQrCode(String qrcodeString, int tam, int posY, int posX) {
        return E1_Display.ApresentaQrCode(qrcodeString, tam, posY, posX);
    }
