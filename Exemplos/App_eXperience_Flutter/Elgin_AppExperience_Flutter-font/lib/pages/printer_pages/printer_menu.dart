...
class PrinterMenutPageState extends State<PrinterMenutPage> {
  ...
  static const String EXTERNAL_PRINTER_MODEL_I7PLUS = "i7 PLUS";
  ...
  onChangedImp(String value) async {
    ...
    if (value != "IMP. INTERNA") {
      List<String> externalPrinterModels = [
        if (value != 'IMP. EXTERNA - IP') ...{EXTERNAL_PRINTER_MODEL_I7PLUS},
      ...
      onExternalPrinterModelTapped(int indexOfTapped) {
        ...
        selectedModel = externalPrinterModels.elementAt(indexOfTapped);
      }
  ...
  Future<bool> connectExternalImpByIP(String model) async {
    ...
    int result = await printerService.connectExternalImpByIP(
        model: model, ip: ip, port: int.parse(port));
