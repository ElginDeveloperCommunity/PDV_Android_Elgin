...
class _BridgePageState extends State<BridgePage> {
  ...
  TextEditingController controllerIpBridge =
      new TextEditingController(text: "192.168.0.100");
  TextEditingController controllerTransactionPort =
      new TextEditingController(text: "3000");
  TextEditingController controllerStatusPort =
      new TextEditingController(text: "3001");
  TextEditingController controllerTransactionValue =
      new TextEditingController(text: "20,00");
  ...
  onConsultTerminalStatusPressed() async {
    ...
    if (tryToUpdateBridgeServer()) {
      ...
      await GeneralWidgets.showAlertDialog(
          mainWidgetContext: context,
          dialogTitle: 'Retorno E1 - BRIDGE',
          dialogText: bridgeReturn);
    }
  }
  ...
  onConsultLastTransactionPressed() async {
    ...
    if (tryToUpdateBridgeServer()) {
      ...
      bridgeReturn =
          await bridgeService.consultarUltimaTransacao(pdv: PDV_CODE);
  ...
  onSendTransactionPressed() async {
    if (tryToUpdateBridgeServer() && isValueValidToElginPay()) {
      ...
      if (selectedPaymentMethod == PaymentMethod.DEBITO) {
        bridgeReturn = await bridgeService.iniciaVendaDebito(
            idTransacao: generateRandomIntForBridgeTransaction(),
            pdv: PDV_CODE,
            valorTotal: transactionValueFormatted);
        ...
        if (isInstallmentsFieldValid()) {
          bridgeReturn = await bridgeService.iniciaVendaCredito(
              idTransacao: generateRandomIntForBridgeTransaction(),
              pdv: PDV_CODE,
              valorTotal: transactionValueFormatted,
              tipoFinanciamento: selectedInstallmentMethod,
              numeroParcelas: int.parse(controllerNumberOfInstallments.text));
        ...
        if (pressedAction == 1) {
          ...
          bridgeService
              .iniciaCancelamentoVenda(
                  ...
                  nsu: inputRef)
              .then((stringRetornoCancelamentoVenda) {
            GeneralWidgets.showAlertDialog(
                mainWidgetContext: context,
                dialogTitle: 'Retorno E1 - BRIDGE',
                dialogText: stringRetornoCancelamentoVenda);
          });
        }
      }
    ...
    if (tryToUpdateBridgeServer()) {
      ...
      onEnableOrDisablePasswordOptionTapped(int indexOfTapped) {
          ...
          onPressedAction(int pressedAction) {
            ...
            if (pressedAction == 1) {
              ...
              if (passwordToBeSet.isEmpty) {
                ...
                bridgeService
                    .setSenhaServer(
                        senha: passwordToBeSet,
                        habilitada: HABILITAR_SENHA_TERMINAL)
                    .then((stringRetornoSetSenhaServer) {
                  GeneralWidgets.showAlertDialog(
                      mainWidgetContext: context,
                      dialogTitle: 'Retorno E1 - BRIDGE',
                      dialogText: stringRetornoSetSenhaServer);
                });
              }
            }
          }
        ...
        //Se a opção selecionada for de desabilite de senha
          ...
          bridgeService
              .setSenhaServer(
                  senha: ERASE_TERMINAL_PASSWORD,
                  habilitada: DESABILITAR_SENHA_TERMINAL)
              .then((stringRetornoSetSenhaServer) {
            GeneralWidgets.showAlertDialog(
                mainWidgetContext: context,
                dialogTitle: 'Retorno E1 - BRIDGE',
      ...
      onAdministrativeOptionTapped(int indexOfTapped) {
        ...
        bridgeService
            .iniciaOperacaoAdministrativa(
                idTransacao: generateRandomIntForBridgeTransaction(),
                pdv: PDV_CODE,
                operacao: indexOfTapped)
            .then(((stringRetornoOperacaoAdministrativa) {
          GeneralWidgets.showAlertDialog(
              mainWidgetContext: context,
              dialogTitle: 'Retorno E1 - BRIDGE',
              dialogText: stringRetornoOperacaoAdministrativa);
        }));
      }
    ...
    if (tryToUpdateBridgeServer()) {
      ...
      onCouponOptionTapped(int indexOfTapped) {
        ...
        switch (indexOfTapped) {
          case 0:
            const String XML_EXEMPLO_CUPOM_NFCE =
            ...
            bridgeService
                .imprimirCupomNfce(
                    xml: XML_EXEMPLO_CUPOM_NFCE,
                    indexcsc: INDEXCSC_EXEMPLO,
                    csc: CSC_EXEMPLO)
                .then((stringRetornoImprimirCupomNfce) {
              GeneralWidgets.showAlertDialog(
                  mainWidgetContext: context,
                  dialogTitle: 'Retorno E1 - BRIDGE',
                  dialogText: stringRetornoImprimirCupomNfce);
            });

            break;
          case 1:
            const String XML_CUPOM_EXEMPLO_SAT =
            ...
            bridgeService
                .imprimirCupomSat(xml: XML_CUPOM_EXEMPLO_SAT)
                .then((stringRetornoImprimirCupomSat) {
              GeneralWidgets.showAlertDialog(
                  mainWidgetContext: context,
                  dialogTitle: 'Retorno E1 - BRIDGE',
                  dialogText: stringRetornoImprimirCupomSat);
            });
          ...
          case 2:
            ...
            const String ASS_QR_CODE_EXEMPLO =
            ...
            bridgeService
                .imprimirCupomSatCancelamento(
                    xml: XML_CUPOM_EXEMPLO_SAT_CANCELAMENTO,
                    assQRCode: ASS_QR_CODE_EXEMPLO)
                .then((stringRetornoImprimirCupomSatCancelamento) {
              GeneralWidgets.showAlertDialog(
                  mainWidgetContext: context,
                  dialogTitle: 'Retorno E1 - BRIDGE',
                  dialogText: stringRetornoImprimirCupomSatCancelamento);
            });
      ...
      onPressedAction(int pressedAction) {
        ...
        if (pressedAction == PRESSED_ACTION_OK) {
          ...
          try {
            ...
            bridgeService
                .setTimeout(timeout: newTransactionTimetouInSeconds)
                .then((stringRetornoSetTimeout) {
              GeneralWidgets.showAlertDialog(
                  mainWidgetContext: context,
                  dialogTitle: 'Retorno E1 - BRIDGE',
...
//Função que valida o IP e as portas de comunicação e transação do Bridge
  bool tryToUpdateBridgeServer() {
    ...
    if (isIpValid() && isTransactionPortValid() && isStatusPortValid()) {
      bridgeService.setServer(
          ipTerminal: controllerIpBridge.text,
          portaTransacao: int.parse(controllerTransactionPort.text),
          portaStatus: int.parse(controllerStatusPort.text));
      return true;
    }
    return false;
  }
  ...
  shouldSendPassword() {
    ...
    if (sendPassword) {
      ...
      bridgeService.setSenha(
          senha: passwordEntered, habilitada: ENABLE_PASSWORD_SUBMISSION);
    } else {
      const bool DISABLE_PASSWORD_SUBMISSION = false;

      bridgeService.setSenha(
          senha: passwordEntered, habilitada: DISABLE_PASSWORD_SUBMISSION);
    }
  }
