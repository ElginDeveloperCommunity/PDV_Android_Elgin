...
const Bridge =()=>{
    ...
    const [isCancelationDialogVisible,setIsCancelationDialogVisible] = useState(false);
    ...
    const [selectedPasswordConfig,setSelectedPasswordConfig]=useState("enablePassword");
    const [isInputPasswordDialogVisible,setIsInputPasswordDialogVisible]=useState(false);
    ...
    const checkBoxType = [
        {id:'passwordSender', textButton: 'ENVIAR SENHA NAS TRANSAÇÕES', value:sendPassword ,setValue: (value) => setSendPassword(value)}
    ]

    const consultButtons=[
        {id:'terminalStatus', textButton: 'CONSULTAR STATUS DO TERMINAL', onPress: () => checkTerminalStatus()},
        {id:'timeOutConfig', textButton: 'CONSULTAR TIMEOUT CONFIGURADO', onPress: () => checkConfiguredTimeout()},
        {id:'lastTransition', textButton: 'CONSULTAR ULTIMA TRANSAÇÃO', onPress: () => checkLastTransaction()},

    ]  

    const configButtons=[
        {id:'terminalPassword', textButton: 'CONFIGURAR SENHA DO TERMINAL', onPress: () => setIsPasswordDialogVisible(true)},
        {id:'transactionTimeOut', textButton: 'CONFIGURAR TIMEOUT PARA TRANSAÇÕES', onPress: () => setIsTimeOutInputVisible(true)},
    ...
    function sendTransaction(){
        if(tryToUpdateBridgeServer() && isValueValidToElginPay() && isInstallmentsFieldValid()){
            ...
            if(paymentMeth == "Crédito"){
                ...
                let actualEvent = DeviceEventEmitter.addListener('eventVendaCredito',event => {
                ...
                let actualEvent=DeviceEventEmitter.addListener('eventVendaDebito',event =>{
    ...
    function cancelTransaction(){
        if(refCode!=""){
            ...
            if(tryToUpdateBridgeServer()){
                ...
                let actualEvent = DeviceEventEmitter.addListener('eventCancelamentoVenda',event => {
    ...
    function admOperation(){
        ...
        if(tryToUpdateBridgeServer()){
            ...
            let actualEvent = DeviceEventEmitter.addListener('eventOperacaoAdministrativa',event => {
    ...
    function doBridgeXmlNFCe(){
        ...
        let actualEvent = DeviceEventEmitter.addListener('eventBridgeNFCe',event => {
    ...
    function doBridgeXmlSAT(){
        ...
        let actualEvent = DeviceEventEmitter.addListener('eventBridgeSAT',event => {
    ...
    function doBridgeXmlCancelment(){
        ...
        let actualEvent = DeviceEventEmitter.addListener('eventBridgeCancelation',event => {
    ...
    function checkTerminalStatus(){
        if(tryToUpdateBridgeServer()){
            ...
            let actualEvent = DeviceEventEmitter.addListener('eventConsultarStatus',event => {
    ...
    function checkConfiguredTimeout(){
        if(tryToUpdateBridgeServer()){
            ...
            let actualEvent = DeviceEventEmitter.addListener('eventGetTimeOut',event => {
    ...
    function checkLastTransaction(){
        if(tryToUpdateBridgeServer()){
            ...
            let actualEvent = DeviceEventEmitter.addListener('eventUltimaTransacao',event => {
    ...
    function configureTerminalPassword(){
        if(selectedPasswordConfig=="enablePassword"){
            ...
            if(!sendPassword){
                Alert.alert("Alerta","Habilite a opção de envio de senha e envie a senha mais atual para desabilitar a senha do terminal");
            }
            else{
                if(tryToUpdateBridgeServer()){
                    shouldSendPassword();
                    ...
                    let actualEvent = DeviceEventEmitter.addListener('eventSetSenhaServer',event => {
    ...
    function enableTerminalPassword(){
        if(passwordEntered==""){
            Alert.alert("Erro na Senha","Por favor insira um valor para a senha desejada!");
        }else{
            if(tryToUpdateBridgeServer()){
                shouldSendPassword();
    ...
    function setTransactionTimeOut(){
        if(newTimeOut==""){
            ...
            if(tryToUpdateBridgeServer()){
                shouldSendPassword();
                bridgeService.sendSetTimeOut(
                    parseInt(newTimeOut)
                );

                let actualEvent = DeviceEventEmitter.addListener('eventSetTimeOut',event => {
    ...
    function isIpAdressValid(){
        ...
        if((/^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]){1}$/.test(numIP))){
            ipValid = true;
            return ipValid;
        }else{
            ipValid = false;
            Alert.alert("Erro de IP","O endereço IP está incorreto, digite um endereço válido");
            return ipValid;
        };
    };
    ...
    function isValueValidToElginPay(){
        if(valor < 1){
            Alert.alert("Erro na entrada de valor","O valor mínimo para transações é de R$1.00!");
            return false;
        }
        else{
            return true;
    ...
    function isInstallmentsFieldValid(){
        if(paymentMeth=="Crédito"){
            if((installmentType == "2"||installmentType =="3")&& parseInt(numParcelas) <2){
                Alert.alert("Erro no parcelamento","O número de parcelas deve ser maior que 2!");
                return false
            }else if (numParcelas==""){
                Alert.alert("Erro no parcelamento","O número de parcelas não pode ser vazio!");
                return false;
            }else{
                return true;
            }
    ...
    function isTransactionPortValid(){
        if(parseInt(trasactionPort)>65535){
            Alert.alert("Erro na Porta de Transação","O valor inserido na porta de transação excede o limite esbelecido de 65535!","OK");
            return false;
        }else if(trasactionPort ==" "){
            Alert.alert("Erro na Porta de Transação","O valor inserido na porta de transação não pode ser vazio");
            return false;
        }else{
            return true;
        }
    ...
    function isStatusPortValid(){
        if(parseInt(trasactionPort)>65535){
            Alert.alert("Erro na Porta de Status","O valor inserido na porta de status excede o limite esbelecido de 65535!");
            return false;
        }else if(trasactionPort ==" "){
            Alert.alert("Erro na Porta de Status","O valor inserido na porta de status não pode ser vazio");
            return false;
        }else{
            return true;
        }
                ...
                <View style={styles.configView}>
                    ...
                    <View>
                        <Dialog.Container visible={isCancelationDialogVisible}>
                            ...
                            keyboardType={'numeric'}
                            ></Dialog.Input>
                            <Dialog.Button label="CANCELAR" onPress={()=>closeDialog('cancelationDialog')}/>
                    ...
                    <View>
                        <Dialog.Container visible={isAdmDialogVisible}>
                            ...
                            <Dialog.Button label="CANCELAR" onPress={()=>setIsAdmDialogVisible(false)}/>
                    ...
                    <View>
                        <Dialog.Container visible={isCuponDialogVisible}>
                            ...
                            <Picker
                                ...
                                <Picker.Item label="Imprimir Cupom Sat Cancelamento" value="cancelCoupon" />
                            </Picker>
                          
                            <Dialog.Button label="CANCELAR" onPress={()=>setIsCuponDialogVisible(false)}/>
                            <Dialog.Button label="OK" onPress={()=>printCouponTest(selectedCoupon)}/>
                        </Dialog.Container>
                    ...
                    <View style={styles.submitionButtonsView}>
                        <TouchableOpacity style={styles.submitionButton} onPress={() => sendTransaction()} >
                            <Text style={styles.textButton}>
                                ENVIAR TRANSAÇÃO
                            </Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.submitionButton} onPress={() => setIsCancelationDialogVisible(true)} >
                            <Text style={styles.textButton}>
                                CANCELAR TRANSAÇÃO
                            </Text>
                        </TouchableOpacity>
                   ...
                   <View style={styles.submitionButtonsView}>
                        <TouchableOpacity style={styles.submitionButton} onPress={() => setIsAdmDialogVisible(true)} >
                            <Text style={styles.textButton}>
                                OPERAÇÃO ADM
                            </Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.submitionButton} onPress={() => setIsCuponDialogVisible(true)}>
                            <Text style={styles.textButton}>
                    ...
                    <View style={styles.e1BridgeFunctionsView}>
                        ...
                        <Dialog.Container visible={isPasswordDialogVisible}>
                            ...
                            <Picker
                                ...
                                <Picker.Item label="Habilitar Senha no Terminal" value="enablePassword" />
                                <Picker.Item label="Desabilitar Senha no Terminal" value="unablePassword" />
                                
                            </Picker>
                          
                            <Dialog.Button label="CANCELAR" onPress={()=>closeDialog('passwordConfigDialog')}/>
                            <Dialog.Button label="OK" onPress={()=>configureTerminalPassword()}/>
                        </Dialog.Container>

                        <View>
                            <Dialog.Container visible={isInputPasswordDialogVisible}>
                                <Dialog.Title>Digite a senha a ser habilitada</Dialog.Title>
                                <Dialog.Input label="Senha: "
                                ...
                                password={true}
                                ></Dialog.Input>
                                <Dialog.Button label="CANCELAR" onPress={()=>closeDialog('passwordInputDialog')}/>
                                <Dialog.Button label="OK" onPress={()=>enableTerminalPassword()}/>
                            </Dialog.Container>
                        </View>

                        <View>
                            <Dialog.Container visible={isTimeOutInputVisible}>
                                <Dialog.Title>Defina um novo timeout para transação (em segundos)</Dialog.Title>
                                <Dialog.Input label="Novo TimeOut: "
                                onChangeText={setNewTimeOut}
                                value={newTimeOut}  
                                keyboardType={'numeric'}
                                ></Dialog.Input>
                                <Dialog.Button label="CANCELAR" onPress={()=>closeDialog('timeOutInputDialog')}/>
                                <Dialog.Button label="OK" onPress={()=>setTransactionTimeOut()}/>
                            </Dialog.Container>
                        </View>         
